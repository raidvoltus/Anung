...
def calculate_indicators(df):
    ...
    df['Volume'] = df['Volume']  # Tetap disimpan untuk evaluasi konfirmasi volume
    df.dropna(inplace=True)
    return df
...
def generate_features(df):
    ...
    features = df[['Return', 'MA20', 'MA50', 'RSI', 'MACD', 'MACD_Signal', 'BB_High', 'BB_Low', 'Volume']]
...
def analyze_stock(symbol):
    ...
    df = calculate_indicators(df)  # Hitung indikator teknikal tambahan
    ...
    target_price = close_price * (1 + TP_MULTIPLIER / 100)
    stop_loss = close_price * (1 - SL_MULTIPLIER / 100)

    if prob_high > MIN_PROBABILITY:
        return {
            'symbol': symbol,
            'action': 'BUY',
...
# Self-learning model evaluation
