...
def analyze_stock(ticker):
    ...
    x_latest = X.iloc[-1:]
    pred_high = model_high.predict(x_latest.values)[0]
    pred_low = model_low.predict(x_latest.values)[0]
    lstm_pred = model_lstm.predict(np.reshape(x_latest.values, (1, x_latest.shape[1], 1)))[0][0]

    current_price = df["Close"].iloc[-1]
    reward = pred_high - current_price
    risk = current_price - pred_low
    profit_pct = (reward / current_price) * 100 if current_price != 0 else 0

    # Estimasi probabilitas dari LightGBM menggunakan variance antar prediksi
...
if __name__ == "__main__":
    ...
    top_5 = sorted(results, key=lambda x: x["profit_pct"], reverse=True)[:5]
    if top_5:
        message = "<b>📊 Top 5 Sinyal Trading Hari Ini:</b>\n"
        for r in top_5:
            message += (f"\n🔹 {r['ticker']}\n   💰 Harga: {r['harga']:.2f}\n   "
                        f"🎯 TP: {r['take_profit']:.2f}\n   🛑 SL: {r['stop_loss']:.2f}\n   "
                        f"📈 Profit: {r['profit_pct']}%\n   🔒 Prob: {r['probabilitas']}\n"
